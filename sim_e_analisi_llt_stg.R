#Simulazione di serie modellate con LLT e stagionalità ed analisi con LODE, auxres e KFS
...
for(j in 1:2){
  set.seed(1152021+j)
  if(j==91|j==2|j==8){#cambio alcune serie perchè danno problemi con KFS
    set.seed(j+2)
  } 
  n<-100
  ...
  b<-cumsum(rnorm(n, sd = sig_beta))+runif(1,-5,5)#slope
  b<-b*sig_eps/3
  
  per<-4
  stg<-numeric()#stagionalita
  stg[1:(per-1)]<-c(runif(1,-5,5),runif(1,-5,5),runif(1,-5,5))
  for(i in per:n){
    stg[i]<--sum(stg[(i-per+1):(i-1)])+rnorm(1,0,sig_stg)
  }
  stg<-stg*sig_eps
  mu <- cumsum(eta+b)+runif(1,-100*sig_eps,100*sig_eps) #aggiungo puntp iniziale diverso
  y <- mu + eps+stg
  ...
  mod <- SSModel(y ~ SSMtrend(2, list(array(0, c(1, 1, (n))),0), a1 = c(y[1],0), P1 = c(var(y),0,0,var(y)), P1inf = c(0,0,0,0))+SSMseasonal(per,1,"dummy"),
                 H = array(0, c(1, 1, n)))
  mle <- optim(c(sd(diff(diff(y,lag=per)))/100, sd(diff(diff(y,lag=per)))/100, sd(diff(diff(y,lag=per)))/100, sd(diff(diff(y,lag=per)))/100), loglik, mod = mod, method = "L-BFGS-B")#,lower = -sd(y),upper=+sd(y))
  y1[j]<-y[1]
  risc[j]<-abs(mle$par[1])
  y<-(y-y[1])*5/abs(mle$par[1])
  mod <- SSModel(y ~ SSMtrend(2, list(array(0, c(1, 1, (n))),0), a1 = c(0,0), P1 = c(var(y),0,0,var(y)), P1inf = c(0,0,0,0))+SSMseasonal(per,1,"dummy"),
                 H = array(0, c(1, 1, n)))
  mle <- optim(c(sd(diff(diff(y,lag=per)))/100, sd(diff(diff(y,lag=per)))/100, sd(diff(diff(y,lag=per)))/100, sd(diff(diff(y,lag=per)))/100), loglik, mod = mod, method = "L-BFGS-B")#,lower = -sd(y),upper=+sd(y))
  ...
  fun_iniz_dom<-function(y){
    ...
    #iniz con smoother
    kfs_dist <- KFS(mod_mle,smoothin="disturbance")
  ...
  l2opt<-ar[1,2]
  foo2 <- try(optim(iniz_dom$iniziali, obj, grad,mod = mod, l1 = l1opt, l2 = l2opt,method = c("BFGS"),control = list(maxit = 50*2*n,factr=1e-10)),silent=TRUE)
  if (class(foo2) == "try-error"){
    foo2 <- try(optim(iniz_dom$iniziali, obj, grad,mod = mod, l1 = l1opt, l2 = l2opt,lower=iniz_dom$low,upper=iniz_dom$up,method = c("L-BFGS-B"),control = list(maxit = 50*2*n,factr=1e-10)))
  }
  foo2$loglik <- make_mod(foo2$par, mod, TRUE)$kfs$logLik
  opts2[[paste0("j=",j)]] <-  foo2
  val2[[paste0("j=",j)]] <-  foo2$value
}
...
for(i in 1:m){
  ...
  updtfn <- function(pars, model) {
    ...
    model$Q[3, 3, ] <- exp(pars[4])
    model
  }
  init <- c(log(mle$par^2)) # initial values
  fit1 <- fitSSM(mod, init, updtfn)
  ...
  #LODE50
  x<-opts1[[i]]
  bicnoi[i]<--2*x$loglik + log(n)*k2*sum(abs(x$par[1:(n+1)])>=abs(x$par[1]/k1))+log(n)*k2*sum(abs(x$par[(n+2):(2*n+2)])>=abs(x$par[(n+2)]/k1))
  bst_lasso_mu1[i,] <-with(opts1[[i]], make_mod(par, mod)$kfs$alphahat[, 1])
  erbest1[i]<-mean((tsmu[i,] - bst_lasso_mu1[i,])^2)
  #LODE30
  x<-opts2[[i]]
  bicnoi2[i]<--2*x$loglik + log(n)*k2*sum(abs(x$par[1:(n+1)])>=abs(x$par[1]/k1))+log(n)*k2*sum(abs(x$par[(n+2):(2*n+2)])>=abs(x$par[(n+2)]/k1))
  bst_lasso_mu2[i,] <-with(opts2[[i]], make_mod(par, mod)$kfs$alphahat[, 1])
  erbest2[i]<-mean((tsmu[i,] - bst_lasso_mu2[i,])^2)
  
  
  #auxres
  dsmo1 <- KFS(fit1$model, smoothing = "disturbance")
  auxres_level <- rstandard(dsmo1, "state")[,1]
  smo1 <- KFS(fit1$model)
  ...
  if (length(ndx_ao)==0){
    AO<-matrix(0,nrow=length(y),ncol=1)
  } else {
  ...
  #level-shift
  ndx <- which(abs(auxres_level[]) > qnorm(pv))
  
  if (length(ndx)==0){
    shift<-matrix(0,nrow=length(y),ncol=1)
    max<-numeric()
  } else {
  ...
  if(length(ndx_ao)==0){   
    mod2 <- SSModel(y ~ shift+SSMtrend(2, list(array(0, c(1, 1, (n))),0), a1 = c(y[1],0), P1 = c(var(y),0,0,var(y)), P1inf = c(0,0,0,0))+SSMseasonal(per,1,"dummy"),
                    H = array(0, c(1, 1, n)))
    
  }else if(length(ndx)==0){
    mod2 <- SSModel(y ~ AO+SSMtrend(2, list(array(0, c(1, 1, (n))),0), a1 = c(y[1],0), P1 = c(var(y),0,0,var(y)), P1inf = c(0,0,0,0))+SSMseasonal(per,1,"dummy"),
                    H = array(0, c(1, 1, n)))
    
  }else{ 
    mod2 <- SSModel(y ~ AO+shift+SSMtrend(2, list(array(0, c(1, 1, (n))),0), a1 = c(y[1],0), P1 = c(var(y),0,0,var(y)), P1inf = c(0,0,0,0))+SSMseasonal(per,1,"dummy"),
                    H = array(0, c(1, 1, n)))
  ...
  if(class(ssmo2)=="try-error"){
    if(length(ndx_ao)==0){   
      ...
      diag(mod2$P1)[1:length(max)]<-var(y)
    }else if(length(ndx)==0){
      mod2 <- SSModel(y ~ AO+SSMtrend(2, list(array(0, c(1, 1, (n))),0), a1 = c(y[1],0), P1 = c(var(y),0,0,var(y)), P1inf = c(0,0,0,0))+SSMseasonal(per,1,"dummy"),
                      H = array(0, c(1, 1, n)))
      diag(mod2$P1inf)[1:length(ndx_ao)]<-0
      diag(mod2$P1)[1:length(ndx_ao)]<-var(y)
    }else{ 
      mod2 <- SSModel(y ~ AO+shift+SSMtrend(2, list(array(0, c(1, 1, (n))),0), a1 = c(y[1],0), P1 = c(var(y),0,0,var(y)), P1inf = c(0,0,0,0))+SSMseasonal(per,1,"dummy"),
                      H = array(0, c(1, 1, n)))
      diag(mod2$P1inf)[(1):(length(ndx_ao)+length(max))]<-0
      diag(mod2$P1)[(1):(length(ndx_ao)+length(max))]<-var(y)
    }
    fit2 <- fitSSM(model=mod2, inits=fit1$optim.out$par, updatefn=updtfn,control=list(factr=1e-10))
    options(warn=1)
    ssmo2 <- try(KFS(fit2$model, smoothing="state"))
  }
  ...
  if(length(max)==1){
    shift <- as.numeric(ssmo2$alphahat[,(length(ndx_ao)+1):(length(ndx_ao)+length(max))]) * t(as.numeric(fit2$model$Z[1,(length(ndx_ao)+1):(length(ndx_ao)+length(max)),]))
...
#MSE-----
#tutte
g<-c(19,85)#elimino due serie dal conto in quanto auxres ha problemi nella stima del primo punto
eauxres<-erauxres[-g]/err[-g] #auxres
elode50<-erbest1[-g]/err[-g] #LODE50
elode30<-erbest2[-g]/err[-g] #LODE30
mean(eauxres[])
mean(elode50[])
mean(elode30[])
mean(bicnoi2-bicnoi)
...
#SW<5%
wsw_lev<-which(sw_lev[-g]<0.05)
wsw_y<-which(sw_y[-g]<0.05)
